...
function Profile({ userId, userData, setUserData, setUserId }) {
    ...
    return (
        <>
            <div className='flex flex-row items-center mb-4 font-FallingSky'>
                <Link to='/' className='m-6'>⬅ Back</Link>
            ...
            <div className="bg-white shadow rounded-lg p-6 font-FallingSky">
                <div className="flex flex-col items-center">
                    <div className="flex items-center">
                        <img src='/Profile.webp' className='h-24 rounded-full m-4' alt="Profile" />
                        <div className='flex flex-col m-6'>
                            <h1 className="text-xl mb-4 font-semibold">{userData.username}</h1>
                            <h2 className="text-gray-600">Amount of Markers posted: {userData.markers?.length}</h2>
                            <h2 className="text-gray-600">This user has {userData.friends?.length} friends</h2>
                            <h2 className="text-gray-600">This user has favorited {userData.favorites?.length} markers</h2>
                        </div>
                    </div>
            ...
            <div className='m-12 rounded-lg'>
                <MapContainer center={[39.8283, -98.5795]} zoom={5} className="h-96 w-full">
                    <TileLayer
